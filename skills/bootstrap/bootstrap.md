...
7. **Uniwind** — `src/global.css` (imports only; layout/spacing/color tokens under `theme/tokens/generated/`); import in root `_layout`; `withUniwindConfig` outermost; `bunx uniwind generate-artifacts --css ./src/global.css --dts ./src/uniwind-types.d.ts` (add `--theme <scheme>` when multi-scheme).
...
1. Review tokens source (GitHub URL or local JSON); **auto-detect** appearance vs color schemes ([TOKEN_SYNC.md](https://github.com/hosam-hubspire/expo-project-bootstrap/blob/main/templates/TOKEN_SYNC.md)) — ask only if ambiguous **or** no named Default for light/dark appearance pin
2. Implement `transformAndWrite` → Uniwind `generated/` (aliases, hex+rgba, stroke/padding/responsive, sm/md/lg+, coverage gate). Pin `APPEARANCE_SCHEME_MAP` in the sync script: named Default → both `default`; else **ask user** which scheme backs light and dark, then pin. Script must not prompt. When Storybook on: emit `src/stories/design-tokens/token-definitions.ts` per [STORYBOOK_TOKEN_DEFINITIONS.md](https://github.com/hosam-hubspire/expo-project-bootstrap/blob/main/templates/STORYBOOK_TOKEN_DEFINITIONS.md). Always emit `colorTokenNames` + `tokenAppearance` (schemes may be empty only in the pre-B stub). Color **values** go in `theme.css` only.
3. Wire UI from detection: `colorScheme` when ≥2 schemes; appearance Settings only when `light-and-dark`; light-only → lock / hide dark. Also: Expo `ThemeProvider` via `isDarkUniwindTheme` (`dark` / `*-night`); Storybook theme bar from `tokenAppearance.schemes`; Colors swatches via `useCSSVariable`
...
6. Phase R **Design token import gaps** section (required when sync on): report source→generated coverage and any missing schemes/colors/sizes/typography, unresolved aliases, skipped collections, fonts not loaded, Settings/`extraThemes` / Storybook theme bar / `ThemeProvider` wiring gaps, or app references to absent stub token names — [SKILL.md](SKILL.md) run report template
...
**IMPORTANT — No prior-session reuse:** Build only from this workflow + current `templates/` + the intake tokens source (GitHub URL or local JSON). Never copy/reconstruct from previous chats, agent transcripts, sibling projects, or an earlier bootstrap of the same app name.
