...
- Auth hook → `src/hooks/use-storage-state.ts`; constants → `src/constants/`. Never hooks under `src/lib/`. Theme color for native props → `src/hooks/use-token-color.ts` (not under `theme/`)
...
- Uniwind: CSS `src/global.css`; Metro `withUniwindConfig` outermost; `bunx uniwind generate-artifacts …` before Phase C. For native color props (`Icon`, `ActivityIndicator`, tab bar, etc.) use `useTokenColor` / `useColorTokens` — **never** dynamic `` `accent-${token}` `` (build-time class detection fails). Resolve via `colorSchemes[theme]` for product schemes and `colorTokens.light|dark` for appearance — do not assume `theme` is only `light`/`dark`. Expo Router `ThemeProvider`: `isDarkUniwindTheme(theme)` (`dark` or `*-night`) — never `theme === "dark"` alone
