...
**Tokens off:** stub `generated/` only; no token scripts. **On:** `sync-design-tokens.mjs` + stub + `tokens:sync`; pin intake **Design tokens source** (GitHub URL or local JSON path). Phase B auto-detects appearance vs color schemes from Figma mode names ([TOKEN_SYNC.md](https://github.com/hosam-hubspire/expo-project-bootstrap/blob/main/templates/TOKEN_SYNC.md)) — exact `light`/`dark` → appearance; other modes (e.g. Default / Rider Tools) → product schemes under light-only. **Never** map arbitrary modes to dark. Pin `APPEARANCE_SCHEME_MAP` from a named Default scheme when present; **if no Default, ask the user during scaffolding** which schemes back light and dark, then pin (script does not prompt). Ask also if mode names are ambiguous.
...
- Token scripts / `tokens:sync` only when sync on; else stub `generated/` only. Phase B: follow [TOKEN_SYNC.md](https://github.com/hosam-hubspire/expo-project-bootstrap/blob/main/templates/TOKEN_SYNC.md) — **auto-detect** appearance vs schemes from mode names (no intake); never map product schemes to dark; `colors.ts` names + metadata (values in `theme.css`); coverage gate; alias resolve incl. semantic→semantic; hex+rgba; size modes sm/md/lg+. Preserve Figma spacing slugs end to end (`space/5` → `--spacing-space-5` → `gap-space-5`), with no stripped numeric alias. **Layout tokens** in generated `spacing.css` `@theme` only (never `global.css`): breakpoints from the design system when present else scaffold 768/1024; **max-width only from source tokens** (e.g. `size/container/*`, `container/*`, `max-width/*` → `--max-width-{slug}`) — **do not invent** scaffold-only keys like `--max-width-content` when syncing; remap or drop app `max-w-content`. Stub `colors.ts` must export `colorTokenNames` + `tokenAppearance` (empty schemes OK) so Phase A typechecks. When Storybook on: regenerate `token-definitions.ts` per [STORYBOOK_TOKEN_DEFINITIONS.md](https://github.com/hosam-hubspire/expo-project-bootstrap/blob/main/templates/STORYBOOK_TOKEN_DEFINITIONS.md); theme bar reads `tokenAppearance.schemes` (no hardcoded chips); Colors swatches use `useCSSVariable`
- Typography Phase B: emit recipes to `generated/typography.ts` and scales to `typography-primitives.css` — `text-size-*` + `leading-*` + `font-Regular|Medium|SemiBold|Bold` from `@theme` — **never** hardcoded `text-[Npx]` / `leading-[Npx]` / `font-normal|bold`. RN custom fonts need weight faces via `expoFontSourceMap` in `src/theme/typography.ts` (see TOKEN_SYNC **Typography (Uniwind)**). Do not invent mono faces unless the tokens repo needs a separate mono stack. **No Figma composite text styles:** retain scaffold `typographyClassNames`, load brand faces, report gap — do not invent style names. Compose overrides with `tailwind-merge` (`mergeTypographyClassName`) so custom size/leading/face utilities replace correctly — always install `tailwind-merge`. Gate leading with `ThemedText` `withLineHeight` (default true; false omits `leading-*`)
...
- Uniwind: CSS `src/global.css` is **imports only** (generated tokens under `theme/tokens/generated/`); Metro `withUniwindConfig` outermost; `bunx uniwind generate-artifacts …` before Phase C (include `--theme` scheme slugs when multi-scheme). For native color props (`Icon`, `ActivityIndicator`, tab bar, etc.) use `useCSSVariable(\`--color-${token}\`)` — **never** dynamic `` `accent-${token}` `` (build-time class detection fails). Expo Router `ThemeProvider`: `isDarkUniwindTheme(theme)` (`dark` or `*-night`) — never `theme === "dark"` alone
...
| App / Settings wiring | appearance · schemes · `extraThemes` · Storybook theme bar · `ThemeProvider`/`isDarkUniwindTheme` | panel hidden incorrectly, Metro themes missing, Storybook still light/dark-only, night schemes keep light nav chrome, … |
