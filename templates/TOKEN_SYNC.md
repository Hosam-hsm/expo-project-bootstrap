...
| `spacing.css` | Size → `@theme` (+ sm / md / lg+ overrides). **Also** Uniwind `--breakpoint-*` and `--max-width-content` (layout tokens — see below). |
...
| Size tokens | Preserve Figma path slugs in CSS names: `space/5` → `--spacing-space-5` → `p-space-5` / `gap-space-5`. Emit one key only—do not also emit a stripped `--spacing-5` alias. Also emit `--radius-*`, `--border-width-*` (strokes), padding as spacing keys if needed, and layout tokens (`--breakpoint-*`, `--max-width-content`) — see **Layout tokens** below. |
| Size modes | Mobile-first: **sm** base; **md** / **lg+** media overrides when the export has size *modes* (not the same as Foundations `breakpoint/*` vars). Use the active `--breakpoint-md` / `--breakpoint-lg` values for those media queries (Figma when present, else scaffold 768 / 1024). |
| Layout tokens | Emit `--breakpoint-*` and `--max-width-content` in generated `spacing.css` `@theme` only — **never** also in `global.css` (duplicate definitions with different values). Prefer Figma Foundations (e.g. `breakpoint/md`, container / max-width tokens) when present; otherwise keep scaffold defaults (`md` 768 · `lg` 1024 · `max-width-content` 800). `global.css` is imports only after Phase A. |
