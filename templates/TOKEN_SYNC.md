...
| `spacing.css` | Size → `@theme` (+ sm / md / lg+ overrides). **Also** layout tokens (`--breakpoint-*`, `--max-width-*`) — see below. |
...
| Size tokens | Preserve Figma path slugs in CSS names: `space/5` → `--spacing-space-5` → `p-space-5` / `gap-space-5`. Emit one key only—do not also emit a stripped `--spacing-5` alias. Also emit `--radius-*`, `--border-width-*` (strokes), padding as spacing keys if needed, and layout tokens — see **Layout tokens** below. |
| Size modes | Mobile-first: **sm** base; **md** / **lg+** media overrides when the export has size *modes* (not the same as `breakpoint/*` vars). Use the active `--breakpoint-md` / `--breakpoint-lg` values for those media queries (design system when present, else scaffold 768 / 1024). |
| Layout tokens | Emit in generated `spacing.css` `@theme` only — **never** also in `global.css`. **Breakpoints (`--breakpoint-*`):** prefer design-system breakpoint tokens when present; if absent, keep scaffold `md` 768 / `lg` 1024 (Uniwind needs *some* values). **Max-width (`--max-width-*`):** emit **only** tokens that exist in the source — common paths include `size/container/*`, `container/*`, `max-width/*`, content-width / layout max tokens — as `--max-width-{figma-slug}` → `max-w-{slug}`. Container-like sizes should use the max-width namespace (not only `--spacing-*`) so `max-w-*` utilities work. **Do not invent** scaffold-only keys (e.g. `--max-width-content: 800px`) during Phase B when the design system has no matching token. Phase A stub may ship `--max-width-content` for demo screens; Phase B must drop that stub key, remap app `max-w-content` to a real generated utility (e.g. the `sm` / default container when several exist), or remove the max-width class if none exist. Report the choice in Phase R. |
